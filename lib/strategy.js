...
export function evaluatePitcher(stats, name, priorStats) {
  // Early season: if current season has < 3 games, fall back to prior season stats
  const workingStats = (stats && stats.games >= 1) ? stats
    : (priorStats && priorStats.games >= 5) ? { ...priorStats, _fromPrior: true }
    : null
  ...
  if (!workingStats) {
    return {
      ...
      keyStats: stats ? `${stats.games} GS — too small a sample` : 'No stats available (pitcher not yet announced or no prior season data)',
      edge: false,
    }
  }

  const stats_ = workingStats
  const seasonNote = stats_._fromPrior ? ' [2025 season]' : ''

  const { era, whip, k9 } = stats_
  const isFade =
    whip !== null && k9 !== null &&
